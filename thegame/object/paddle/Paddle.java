...
public class Paddle {
    ...
    public void expand(int amount, int durationMs) {
        if (!expanded) {
            // tăng kích thước (mình dùng current width + amount để thích nghi với thay đổi động)
            setWidth(this.width + amount);
            expanded = true;
        } else {
            // nếu đã mở rộng thì không cộng thêm nữa (tránh stacking)
            // chỉ gia hạn thời gian bằng cách stop timer cũ và tạo timer mới dưới
        }
        ...
        // Tạo timer mới để trả về kích thước base sau durationMs
        expandTimer = new Timer(durationMs, new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                // Trả về kích thước gốc (baseWidth)
                setWidth(baseWidth);
                expanded = false;
                expandTimer.stop();
            }
